...
class SequentialStage extends Stage {
    SequentialStage(String filename, String stagename, Stage parent, def jenkins) {
        super(filename, stagename, parent, jenkins)
    }

    void execute(Map data) {
        this.checkCircularLoop(this)
        List temp = (List) data.get("stages")
        List executionPlan = []
        List seenNames = []
        ...
        for (each in temp) {
            if (each instanceof String) {
                ...
                executionPlan.add({
                    def child = constructChild(capturedFile, capturedStage)
                    jenkins.stage(child.stagename) { child.run() }
                })
            } else if (each instanceof Map) {
                Map entry = (Map) each
                if (entry.containsKey("template")) {
                    Map result = buildTemplateStage(entry, seenNames, "stages")
                    ...
                    for (r in buildIteratedStages(entry, seenNames, "stages")) {
